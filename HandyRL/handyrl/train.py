...
from handyrl.model import to_torch, to_gpu, ModelWrapper
...
#
# log_dir_name = str(len(os.listdir('D:\\PycharmProjects\\HandyRL-master\\runs')) + 1)
# log_path = 'D:\\PycharmProjects\\HandyRL-master\\runs\\' + log_dir_name
# os.mkdir(log_path)
...
def make_batch(episodes, args):
    ...
    for ep in episodes:
        ...
        obs = [[replace_none(m['observation'][player], obs_zeros) for player in players_] for m, players_ in
               zip(moments, players_list)]
        prob = np.array([[[replace_none(m['selected_prob'][player], 1.0)] for player in players_] for m, players_ in
                         zip(moments, players_list)])
        act = np.array([[replace_none(m['action'][player], 0) for player in players_] for m, players_ in
                        zip(moments, players_list)], dtype=np.int64)[..., np.newaxis]
        amask = np.array(
            [[replace_none(m['action_mask'][player], amask_zeros + 1e32) for player in players_] for m, players_ in
             zip(moments, players_list)])
        ...
        v = np.array([[replace_none(m['value'][player], [0]) for player in players] for m in moments],
                     dtype=np.float32).reshape(len(moments), len(players), -1)
        rew = np.array([[replace_none(m['reward'][player], 0) for player in players] for m in moments],
                       dtype=np.float32).reshape(len(moments), len(players), -1)
        ret = np.array([[replace_none(m['return'][player], 0) for player in players] for m in moments],
                       dtype=np.float32).reshape(len(moments), len(players), -1)
        ...
        tmask = np.array([[[m['selected_prob'][player] is not None] for player in players] for m in moments],
                         dtype=np.float32)
        omask = np.array([[[m['observation'][player] is not None] for player in players] for m in moments],
                         dtype=np.float32)
        ...
        if len(tmask) < batch_steps:
            ...
            obs = map_r(obs, lambda o: np.pad(o, [(pad_len_b, pad_len_a)] + [(0, 0)] * (len(o.shape) - 1), 'constant',
                                              constant_values=0))
            prob = np.pad(prob, [(pad_len_b, pad_len_a), (0, 0), (0, 0)], 'constant', constant_values=1)
            v = np.concatenate([np.pad(v, [(pad_len_b, 0), (0, 0), (0, 0)], 'constant', constant_values=0),
        ...
        for t in range(batch_shape[1]):
            ...
            if t < args['burn_in_steps']:
                ...
                if not model.training:
                    model.train()
                outputs_ = model(obs, hidden_)
            outputs_ = map_r(outputs_,
                             lambda o: o.unflatten(0, (batch_shape[0], batch_shape[2])))  # (..., B, P or 1, ...)
...
def compose_losses(outputs, log_selected_policies, total_advantages, targets, batch, args):
    ...
    entropy_loss = entropy.mul(1 - batch['progress'] * (1 - args['entropy_regularization_decay'])).sum() * -args[
...
def compute_loss(batch, model, hidden, args):
    ...
    if 'value' in outputs_nograd:
        ...
        if args['turn_based_training'] and values_nograd.size(2) == 2:  # two player zerosum game
            ...
            values_nograd = (values_nograd * omasks + values_nograd_opponent * omasks_opponent) / (
                        omasks + omasks_opponent + 1e-8)
    ...
    value_args = outputs_nograd.get('value', None), batch['outcome'], None, args[
        'lambda'], 1, clipped_rhos, cs, value_target_masks
    return_args = outputs_nograd.get('return', None), batch['return'], batch['reward'], args['lambda'], args[
        'gamma'], clipped_rhos, cs, return_target_masks
...
class Trainer:
    ...
    def train(self):
        ...
        #     if 'p' in k:
        #         policy_loss = l / data_cnt
        #     else:
        #         value_loss = l / data_cnt
        #
        # writer.add_scalar('PolicyLoss-Steps', policy_loss, self.steps)
        # writer.add_scalar('ValueLoss-Steps', value_loss, self.steps)
...
class Learner:
    def __init__(self, args, net=None, remote=False):
        ...
        if self.model_epoch > 0:
            (self.
             model.load_state_dict(torch.load(self.model_path(self.model_epoch)), strict=False))
    ...
    def feed_episodes(self, episodes):
        ...
        maximum_episodes = self.args['maximum_episodes'] if mem_ok else int(
            len(self.trainer.episodes) * 95 / mem_percent)
